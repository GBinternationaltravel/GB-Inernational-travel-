...
import { upsertAdminTravelGuide } from "@/services/admin-cms-service";

export async function POST(request: Request) {
  const ip =
    request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "anonymous";
  const limited = rateLimit(`admin:travel-guides-create:${ip}`, { limit: 30, windowMs: 60_000 });
  if (!limited.success) {
  ...
  try {
    ...
    return adminCmsErrorResponse(error, "Could not create travel guide.");
  }
}
